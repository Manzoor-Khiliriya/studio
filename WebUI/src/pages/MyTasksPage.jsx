    ...
    },
    {
      ...
      render: (row) => (
        <span className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border ${
          row.status === "Completed" ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-orange-50 text-orange-500 border-orange-100"
        }`}>
          {row.status}
        </span>
      )
        ...
        return (
          <div className="flex justify-end pr-4">
            <button
              ...
              className={`px-8 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all active:scale-95 border-2 ${
                isRunning 
          ...
          <div className="flex items-center gap-2 bg-slate-100/50 p-2 rounded-[2rem] border border-slate-100">
            {["All", "Pending", "In Progress", "Completed"].map((status) => (
              <button
                ...
                className={`px-8 py-4 rounded-[1.5rem] text-[10px] font-black uppercase tracking-[0.2em] transition-all ${
                  statusFilter === status 
