...
const AdminTaskReportPage = () => {
  ...
  // --- API CALL (Using the Project Slice) ---
        ...
        <div className="space-y-6 mb-10">
          {projectGroups.length > 0 ? (
            projectGroups.map((group) => {
              // Now using projectPerc and totalConsumed directly from backend logic if available
              // or calculating locally:
              ...
              return (
                <div key={group._id} className={`bg-white rounded-[2.5rem] border transition-all duration-300 overflow-hidden ${isExpanded ? 'border-orange-500 shadow-xl' : 'border-slate-200 shadow-sm'}`}>
                  ...
                  >
                    <div className="flex items-center gap-6">
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all ${isExpanded ? 'bg-orange-500 scale-110 shadow-lg shadow-orange-500/40 text-white' : 'bg-slate-100 text-slate-500'}`}>
                        <HiOutlineChartBarSquare size={28} />
                      </div>
                      <div>
                        <h3 className="font-black text-lg uppercase tracking-tight">{group.project_code || "UNKNOWN"}</h3>
                        <p className={`text-[10px] font-black uppercase tracking-widest ${isExpanded ? 'text-slate-500' : 'text-slate-400'}`}>
                          {group.taskList?.length || 0} Active Tasks
                        </p>
                      </div>
                    </div>
                              ...
                              <div key={task._id} className="bg-white px-6 py-5 rounded-2xl border border-slate-200 shadow-sm hover:border-orange-500/30 transition-all group/task">
                                <div className="flex flex-col md:flex-row md:items-center gap-4">
                                  ...
                                  <div className="md:w-1/4 flex items-center gap-3">
                                    <div className="flex-1">
                                      ...
                                      <div className="flex items-center gap-2 mt-1">
                                        <span className={`w-1.5 h-1.5 rounded-full ${isTaskOver ? 'bg-rose-500 animate-pulse' : 'bg-emerald-500'}`} />
                                  ...
                                  <div className="flex-1 flex items-center gap-3">
                                    ...
                                    <span className={`text-[12px] font-black tabular-nums min-w-[35px] text-right ${isTaskOver ? 'text-rose-500' : 'text-slate-900'}`}>
                                      {taskPerc.toFixed(0)}%
                                    </span>
                                  </div>
                                    ...
                                    {isTaskOver && (
                                      <div className="flex flex-col items-end">
                                        ...
                                        <span className="text-[11px] font-black text-rose-600">
                                          +{(task.consumedHours - task.allocatedTime).toFixed(1)}h
                                        </span>
                                      </div>
                                    )}
                                  </div>
