...
const Metric = ({ label, value, colorClass = "text-slate-700" }) => (
  <div className="flex flex-col items-end min-w-[50px]">
    <span className="text-[7px] font-black text-slate-400 uppercase tracking-tighter leading-none mb-0.5">
      {label}
    </span>
    <span className={`text-[10px] font-black tabular-nums ${colorClass}`}>
      {value}
    </span>
  </div>
);
        ...
        <div className="bg-white border border-slate-200 p-5 rounded-[1.5rem] shadow-lg shadow-slate-200/40 mb-6 flex flex-col md:flex-row gap-2 items-center">
          ...
          {searchTerm && (
            <button
              ...
              className="px-4 py-2 text-rose-600 font-black text-[9px] uppercase hover:underline"
            >
              Clear
            </button>
          )}
          ...
          {projectGroups.length > 0 ? (
            projectGroups.map((group) => {
              const projectPerc =
                ...
                ((group.totalConsumed / group.totalBudget) * 100 || 0);
              ...
              return (
                ...
                >
                  ...
                  <div
                    onClick={() =>
                      setExpandedProject(isExpanded ? null : group._id)
                    }
                  ...
                  >
                    <div className="flex items-center gap-4 flex-1">
                      <div
                        className={`w-10 h-10 rounded-xl flex items-center justify-center ${isExpanded ? "bg-orange-500 text-white" : "bg-slate-100 text-slate-400"}`}
                      >
                        <HiOutlineChartBarSquare size={20} />
                      </div>
                      <div className="min-w-[120px]">
                        <h3 className="font-black text-sm uppercase tracking-tighter italic leading-none">
                          {group.title || "N/A"}
                        </h3>
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-[7px] font-black uppercase opacity-50">
                            {group.taskList?.length || 0} Tasks
                          </span>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center gap-6 flex-1">
                      <div className="hidden lg:flex flex-col items-end min-w-[280px] flex-1">
                        <div className="flex justify-between w-full mb-1">
                          ...
                          <span
                            className={`text-[9px] font-black ${isProjectOver ? "text-rose-400" : isExpanded ? "text-white" : "text-slate-900"}`}
                          >
                            {projectPerc.toFixed(1)}%
                          </span>
                        </div>
                        <CustomProgressBar
                          percentage={projectPerc}
                          isOver={isProjectOver}
                        />
                      </div>
                      <HiChevronDown
                        className={`transition-transform ${isExpanded ? "rotate-180 text-orange-500" : "text-slate-300"}`}
                        size={16}
                      />
                    </div>
                  </div>
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="w-[680px] flex items-center gap-4">
                                  ...
                                  />
                                  <span
                                    className={`text-[10px] font-black tabular-nums min-w-[30px] text-right ${isTaskOver ? "text-rose-500" : "text-slate-900"}`}
                                  >
                                    {taskPerc.toFixed(0)}%
                                  </span>
                                </div>
            ...
            <div className="bg-white rounded-[2rem] py-20 text-center border-2 border-dashed border-slate-200">
              <p className="text-slate-300 font-black uppercase italic text-sm">
                No Projects Found
              </p>
            </div>
          )}
