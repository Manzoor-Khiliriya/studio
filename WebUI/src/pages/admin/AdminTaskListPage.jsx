...
export default function AdminTasksPage() {
  ...
  // --- DATA FETCHING ---
  // We fetch by Project. Each project object now contains its own .tasks array
  const { data, isLoading, isFetching } = useGetProjectsQuery({
    page: currentPage,
    limit: limit,
    search: searchTerm,
    // Note: status filter here would filter projects. 
    // If you want to filter tasks inside projects, handle that in the backend populate query.
  });

  // Simplified project list from the API response
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 pb-20">
          ...
          {projectGroups.length > 0 ? (
            projectGroups.map((project) => {
              ...
              return (
                <div
                  key={project._id}
                  className={`bg-white rounded-lg border transition-all duration-300 ${expandedProject === project.project_code
                ...
                >
                  ...
                  <div className="p-6 flex items-center justify-between hover:bg-slate-50/50 transition-colors group/header">
                      ...
                      <div className="flex items-center gap-6">
                        <div className="flex items-center gap-2 text-slate-400">
                          ...
                          <span className="text-[10px] font-black uppercase tracking-widest">
                            Client: <span className="text-slate-900 ml-1">{project.clientName || "Direct"}</span>
                          </span>
                        </div>
                        <div className="flex items-center gap-2 text-slate-400">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div
                        onClick={() => setExpandedProject(expandedProject === project.project_code ? null : project.project_code)}
                        className={`cursor-pointer transition-all duration-500 p-3 rounded-xl ${expandedProject === project.project_code
                  ...
                  {expandedProject === project.project_code && (
                    <div className="bg-white border-t border-slate-50">
                      {tasks.length > 0 ? (
                        ...
                        <div className="py-12 flex flex-col items-center justify-center bg-slate-50/50 rounded-b-lg border-t border-slate-100">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">
                            No task objectives assigned to this project yet.
                          </p>
                        </div>
                      )}
                    </div>
        ...
        <div className="bg-white p-6 rounded-lg border border-slate-200 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
          ...
          <Pagination
            pagination={{
              ...
              count: data?.pagination?.totalProjects, // Tracking project count
              limit: limit,
            }}
      ...
      <TaskModal
        ...
        // Passing the fetched projects to the task modal for selection
