...
export default function AdminTasksPage() {
  ...
  const { data, isLoading, isFetching, refetch } = useGetProjectsQuery(
    {
      ...
      ...(activeTab === "live" && {
        ...
        projectStatus: liveProjectStatus,
      }),
  ...
  return (
    <div className="max-w-[1750px] mx-auto min-h-[83vh] bg-slate-100 overflow-x-hidden">
      <PageHeader
      ...
      <main className="max-w-[1750px] mx-auto px-4 sm:px-6 lg:px-8 pb-6 sm:pb-10 -mt-6 sm:-mt-10 overflow-visible">
        {activeTab !== "Settings" && (
          <div className="bg-white/90 border border-slate-200 p-4 sm:p-5 lg:p-6 rounded-2xl sm:rounded-[2.5rem] shadow-xl shadow-slate-200/50 mb-4 flex flex-col gap-4 sm:gap-5 overflow-visible">
            ...
            <div className="flex flex-col lg:flex-row lg:flex-wrap items-stretch lg:items-end justify-between gap-4 sm:gap-6 pt-4 sm:pt-5 border-t border-slate-100">
              <div className="flex flex-wrap items-end gap-2 sm:gap-3 w-full lg:w-auto">
                {activeTab === "live" && (
                  <div className="flex flex-col gap-1.5 w-[calc(50%-0.25rem)] sm:w-auto">
                    ...
                    <CustomDropdown
                      ...
                      className="min-w-0 w-full sm:min-w-35"
                    />
                ...
                {[
                  ...
                  <div key={filter.key} className="flex flex-col gap-1.5 w-[calc(50%-0.25rem)] sm:w-auto">
                    <label className="text-[9px] font-black text-slate-400 uppercase ml-2 tracking-widest">
                      {filter.label}
                    </label>
                    ...
                    <div className="relative group">
                      ...
                      <input
                        type="date"
                        className="w-full sm:w-auto pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-[10px] font-bold text-slate-700 outline-none focus:border-orange-500 focus:bg-white transition-all shadow-sm"
                        value={
                          activeTab === "live"
                ...
                {activeTab === "live" && user?.role === "Admin" && (
                  <button
                    ...
                    className="flex items-center justify-center gap-2 w-[calc(50%-0.25rem)] sm:w-auto px-3.5 py-2.5 bg-slate-50 border border-slate-100 text-[10px] font-black hover:bg-orange-600 hover:text-white rounded-xl transition-all uppercase tracking-widest shadow-lg shadow-orange-200 cursor-pointer active:scale-95"
                  >
                ...
                <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto">
                  ...
                  {((activeTab === "live" ? liveSearch : allSearch) ||
                    ...
                    (activeTab === "live" && statusFilter !== "All") ||
                    (activeTab === "live" &&
          ...
          activeTab === "Settings" ? (
            <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-6">
              <div className="inline-flex items-center bg-slate-100 p-1 rounded-2xl border border-slate-200 shadow-sm mb-3">
                {["Initiative Status", "Active Status", "Task Status", "Project Type"].map((tab) => (
                  <button
                    key={tab}
                  ...
                  return (
                    ...
                    >
                      <div className="px-3 sm:px-4 lg:px-6 py-3 sm:py-2 flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3 hover:bg-slate-50/80 transition-all group/header border-b border-slate-100 last:border-0">
                        ...
                        <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap justify-end w-full lg:w-auto shrink-0 pt-2 lg:pt-0 border-t lg:border-t-0 border-slate-100">
                          {activeTab === "live" && user?.role === "Admin" && (
                            <button
                              onClick={(e) => { e.stopPropagation(); openTaskModalForProject(project); }}
                              className="flex items-center gap-1.5 mx-0 sm:mx-1.5 px-2.5 sm:px-3.5 py-2 sm:py-3 bg-slate-50 border border-slate-100 text-[9px] sm:text-[10px] font-black hover:bg-orange-600 hover:text-white rounded-xl transition-all uppercase tracking-widest shadow-lg shadow-orange-200 cursor-pointer active:scale-95 shrink-0"
                            >
                          ...
                          {user?.role === "Admin" && (
                            <>
                              {activeTab !== "live" && (
                                <button onClick={(e) => { e.stopPropagation(); setPaymentProject(project); setShowPaymentModal(true); }}
                                  className="p-2 sm:p-3 rounded-xl border border-slate-200 text-emerald-500 hover:text-emerald-600 hover:border-emerald-500 transition-all bg-white shadow-xl cursor-pointer shrink-0"
                                  title="Update Invoice & Payment">
                                  <HiOutlineDocumentText />
                                </button>
                              )}

                              <button onClick={(e) => { e.stopPropagation(); setEditingProject(project); setShowProjectModal(true); }}
                                className="p-2 sm:p-3 rounded-xl border border-slate-200 text-yellow-500 hover:text-yellow-600 hover:border-orange-500 transition-all bg-white shadow-xl cursor-pointer shrink-0"
                                title="Update Project">
                                <HiOutlinePencilSquare />
                              </button>

                              <button onClick={(e) => { e.stopPropagation(); setProjectToDeactivate(project); }}
                                className="p-2 sm:p-3 rounded-xl border border-slate-200 text-rose-500 hover:text-rose-600 hover:border-rose-500 transition-all bg-white shadow-xl cursor-pointer shrink-0"
                                title="Delete Project">
                                <HiOutlineTrash />
                              </button>

                              <div
                                className={`px-2.5 sm:px-3 py-2 sm:py-2.5 flex items-center rounded-xl border hover:border-orange-500 transition-all shrink-0 ${isSelected ? "bg-orange-500 border-orange-500" : "bg-white border-slate-200"}`}
                                onClick={(e) => e.stopPropagation()}
                              >
                                <input type="checkbox" checked={isSelected} onChange={() => toggleProjectSelection(project._id)}
                                  className="w-4 h-5 accent-white cursor-pointer" title="Select To Delete Project" />
                              </div>
                            </>
                          )}

                          {activeTab === "live" && (
                            <div className={`cursor-pointer p-2 ml-1 sm:ml-2 rounded-full text-orange-500 transform transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] shrink-0 ${expandedProject === project.projectCode ? "rotate-180" : "rotate-0"}`}>
