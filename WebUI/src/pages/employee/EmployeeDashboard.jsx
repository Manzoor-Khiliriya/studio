...
export default function EmployeeDashboard() {
  ...
  const handleClockOutConfirm = async () => {
    try {
      const activeLog = logsData?.logs?.find(
        log => log.isRunning && log.logType === "work"
      );
      if (activeLog) {
        await stopTimer(activeLog._id).unwrap();
        setForceStopSignal(prev => prev + 1);
      }
      await clockOut().unwrap();
      setConfirmModal({ isOpen: false, type: null });
      toast.success(
        activeLog
          ? "Task stopped & Clocked out successfully"
  ...
  const hasRunningTask = logsData?.logs?.some(
    log => log.isRunning && log.logType === "work"
  );
  ...
  return (
    <>
      ...
      <ConfirmModal
        ...
        message={
          hasRunningTask
            ? "A task is running. It will be stopped automatically. Are you sure you want to clock out?"
